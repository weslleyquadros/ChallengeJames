...
extension UIView {
    ...
    @available(*, deprecated, message: "This method uses topLayoutGuide, that has been deprecated by Apple. Use fillSuperview instead.")
    @discardableResult
    public func fillSuperViewInViewController(viewController: UIViewController, insets: UIEdgeInsets = .zero) -> Constraints {
        guard let superview = superview else {
             preconditionFailure("🔴 To use fillSuperViewInViewController, view needs to be added to a superview") 
        }
        
        guard #available(iOS 11.0, *) else {
            return anchor (
                top: (viewController.topLayoutGuide.bottomAnchor, insets.top),
                left: (superview.leftAnchor, insets.left),
                right: (superview.rightAnchor, insets.right),
                bottom: (superview.bottomAnchor, insets.bottom)
            )
        }
        
        return anchor (
            top: (viewController.topLayoutGuide.bottomAnchor, insets.top),
            left: (superview.safeAreaLayoutGuide.leftAnchor, insets.left),
            right: (superview.safeAreaLayoutGuide.rightAnchor, insets.right),
            bottom: (superview.safeAreaLayoutGuide.bottomAnchor, insets.bottom)
        )
    }
}
